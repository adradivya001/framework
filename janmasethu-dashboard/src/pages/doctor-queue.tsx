...
import { Stethoscope, AlertCircle, Search } from 'lucide-react';
...
export default function DoctorQueue() {
    ...
    const redThreads = threads
        .filter((t) => t.severity === 'RED' || t.assignedUser === user?.name)
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-slate-900 flex items-center space-x-2">
                                ...
                                <span className="text-sm bg-red-100 text-red-600 font-semibold px-2.5 py-0.5 rounded-full">{redThreads.length} active</span>
                            </h1>
                            <p className="text-sm text-slate-500">Emergency RED threads requiring immediate medical intervention</p>
                        </div>
                    </div>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                        <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)}
                            className="h-9 rounded-lg border border-slate-200 pl-9 pr-4 text-sm focus:border-red-400 focus:outline-none w-52" />
                    </div>
                </div>

                {redThreads.length === 0 ? (
                    <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl py-24 text-center">
                        <AlertCircle size={48} className="mx-auto mb-3 text-slate-200" />
                        <p className="font-medium text-slate-400">No emergency threads at this time.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {redThreads.map((thread) => (
